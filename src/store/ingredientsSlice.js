  ...
  async (_, thunkAPI) => {
    try {
      ...
      res.data.map((element, index) => {
        if (element && measures.find((m) => m.id === element.measureId)) {
          // Check if element and measure are not undefined
          ing.push({
            id: element.id,
            name: element.name,
            recipeId: element.recipeId,
            amount: element.amount,
            measure: measures.find((m) => m.id === element.measureId).name,
    ...
    try {
      ...
      thunkAPI.dispatch(
        pushIngredient({
          ...
          measure: thunkAPI
            .getState()
            .measures.measures.find((m) => m.id === ing.measureId).name,
        })
      );
...
export const editIngredient = createAsyncThunk(
  ...
  async (ing, thunkAPI) => {
    try {
      if (
        thunkAPI.getState().ingredients.ingredients.find((i) => i.id === ing.id)
      ) {
...
export const { pushIngredient } = ingredientsSlice.actions;
